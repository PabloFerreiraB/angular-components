...
import { MatPaginator, PageEvent as MaterialPageEvent } from '@angular/material/paginator';
...
export class DynamicTableComponent implements OnInit, AfterViewInit, AfterContentInit, OnDestroy {
  ...
  private selectRows(indexes: number[]) {
    ...
    this.selection.select(
      ...this.rows.filter((value: any, index: number) => indexes.includes(index))
    );
  ...
  initTable() {
    ...
    if (this.expandedDetail)
      this.displayedColumns.splice(this.positionColumnIconExpandedDetail, 0, 'detail');
  ...
  toggleAllRows(event: any) {
    ...
    const selectable = this.dataSource.data.filter((row) => this.canSelectFn(row, true));
  ...
  checkboxLabel(row?: any): string {
    ...
    return `${this.selection.isSelected(row) ? 'Deselecionar' : 'Selecionar'} linha`;
  }
